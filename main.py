...
def wait_for_response(timeout=60, number=None, station_name=None, retry=True):
    ...
    if not is_received and retry and number:
        ...
        threading.Thread(target=wait_for_response, args=(timeout, number, station_name, False), daemon=True).start()
...
def show_status(msg):
    ...
    if last_sent_number and last_sent_number in station_widgets:
        ...
        label.config(text=msg)
         ...
         # Balansı çıxar və balance_label-a yaz
        balance_match = re.search(r"Balance[-:]?\s*([\d\.]+)\s*Azn", msg, re.IGNORECASE)
        if balance_match:
            balance = balance_match.group(1)
            station_widgets[last_sent_number]["balance_label"].config(text=f"Balans: {balance} Azn")
        else:
            station_widgets[last_sent_number]["balance_label"].config(text="")


        # Stansiya adını tap
        station_name = ""
        for s in load_stations():
            if s["phone"] == last_sent_number:
                station_name = s["station"]
                break
        log_status(station_name, last_sent_number, f"Cavab: {msg}")
...
def send_sms(number, message):
    ...
    for s in load_stations():
        if s["phone"] == number:
            ...
            break
    # Log yazırıq
    log_status(station_name, number, f"Göndərildi: {message}")
...
def send_sms_status(number):
    ...
    t.start()
    messagebox.showwarning(f"Stansiya işləmir: {number}", f"{number} stansiyası 2 dəqiqə ərzində cavab vermədi!\nOK düyməsini basın səsi dayandırmaq üçün.")
...
def draw_station_cards(stations):
    ...
    for i, station in enumerate(stations):
        ...
        # Link üçün kliklənən label
        if "link" in station and station["link"]:
            def open_link(event, url=station["link"]):
                webbrowser.open(url)

            link_label = tk.Label(card, text="Ünvanı aç", font=("Arial", 11, "underline"),
...
tk.Label(root, text="COM port seç:", bg="#f0f0f0", fg="#000", font=("Courier New", 16, "bold")).place(x=10, y=20)
port_dropdown = ttk.Combobox(root, textvariable=port_var, font=("Arial", 11), width=30)
port_dropdown.place(x=180, y=20)
status_label = tk.Label(root, text="Qoşulmayıb", fg="red", bg="#f0f0f0", font=("Arial", 16))
status_label.place(x=20, y=60)

tk.Button(root, text="Yenilə", command=refresh_ports, width=10, bg="#d12b2b").place(x=450, y=18)
tk.Button(root, text="Qoşul", command=connect_serial, width=10, bg="#06a96d").place(x=540, y=18)
tk.Button(root, text="Bağla", command=disconnect_serial, width=10, bg="#5c3538").place(x=630, y=18)
tk.Button(root, text="Stansiya əlavə et", command=open_add_modal, bg="#2b7de9", fg="#000").place(x=1100, y=18)
